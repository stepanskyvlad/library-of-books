...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    def create(email, password, first_name=None, middle_name=None, last_name=None, role=0):
        ...
        if len(first_name) > 20:
            return "First name no more than 20 characters!"
        if len(middle_name) > 20:
            return "Middle name no more than 20 characters!"
        if len(last_name) > 20:
            return "Last name no more than 20 characters!"
        if len(email) > 100:
            return "Email no more than 100 characters!"
